...
if (jsStart === -1) {
    const backupStart = content.indexOf('<script>\n  document.addEventListener("DOMContentLoaded", function() {');
    if (backupStart !== -1) {
        console.log("Found backup start");
    } else {
...
if (jsStart !== -1 && jsEnd !== -1) {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Show playlist panel by default
    ...
    playBtn.addEventListener("click", () => {
      if (!audio.src || audio.src.endsWith('ver')) {
        loadChapter(0);
        return;
      }
      if (audio.paused) {
    ...
    // Auto-load first chapter metadata without playing
