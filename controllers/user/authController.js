  ...
  passport.use(new GoogleStrategy({
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        if (admin) return done(null, { ...admin.toObject(), authRol: 'administrador' });
...
async function sendEmail(to, subject, text) {
  ...
  try {
    const fromEmail = process.env.SMTP_FROM || process.env.SMTP_USER || 'noreply@biblionet.teamfusion.site';
...
exports.registerForm = (req, res) => {
  ...
  res.render('auth/index', { title: 'Registro', layout: false, formData, activeForm: 'register' });
};
...
exports.register = async (req, res, next) => {
  try {
    ...
    if (!PASSWORD_REGEX.test(password)) {
      flash(req, 'error', 'La contraseña debe tener entre 8 y 20 caracteres, una mayúscula, una minúscula, un número y un carácter especial.');
      req.session.formData = { nombre, correo, telefono };
      return res.redirect('/registro');
    }
    ...
    if (existeAdmin || existeUsuario) {
      flash(req, 'error', 'Este correo ya está registrado. Si olvidaste tu contraseña, puedes recuperarla.');
    ...
    try {
      await sendEmail(correoLower, 'Verifica tu cuenta en BiblioNet', `Tu código de verificación es: ${codigo}\n\nEste código caduca en 10 minutos.`);
    } catch (emailError) {
      flash(req, 'error', 'Cuenta creada, pero hubo un error al enviar el correo. Por favor contacta soporte.');
      return res.redirect('/login');
    }
    
    // Guardar ID temporal en sesión para la verificación
    req.session.tempUserId = nuevoUsuario._id;
    flash(req, 'success', `Cuenta creada. Hemos enviado un código de verificación a tu correo: ${correoLower}`);
...
exports.verifyEmailForm = (req, res) => {
  ...
  res.render('auth/index', { title: 'Verificar Correo', layout: false, activeForm: 'verifyEmail' });
};
...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (!usuario.verificationCodeExpires || Date.now() > usuario.verificationCodeExpires) {
      flash(req, 'error', 'El código ha expirado. Por favor, solicita uno nuevo.');
      return res.redirect('/verificar-correo');
    }

    if (usuario.verificationAttempts >= 5) {
      flash(req, 'error', 'Demasiados intentos fallidos. Solicita un código nuevo más tarde.');
      return res.redirect('/verificar-correo');
    }
    ...
    flash(req, 'success', 'Cuenta verificada exitosamente. Ahora puedes iniciar sesión.');
    res.redirect('/login');
  } catch (error) {
    next(error);
  }
...
exports.resendVerificationCode = async (req, res, next) => {
  try {
    ...
    try {
      await sendEmail(usuario.correo, 'Nuevo código de verificación', `Tu nuevo código de verificación es: ${codigo}\n\nEste código caduca en 10 minutos.`);
      flash(req, 'success', `Nuevo código enviado al correo: ${usuario.correo}`);
    } catch (emailError) {
      flash(req, 'error', 'Error al enviar el correo de verificación. Verifica la configuración.');
    }
...
exports.loginForm = (req, res) => {
  if (req.query.expirada === '1' && !res.locals.flash) {
    res.locals.flash = { type: 'info', message: 'Tu sesión expiró después de 15 minutos de inactividad.' };
  }
  res.render('auth/index', { title: 'Iniciar sesión', layout: false, activeForm: 'login' });
>>>>>>> Cambios
};
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (usuario) {
      const passwordMatch = usuario.password_hash ? await bcrypt.compare(password, usuario.password_hash) : false;
      if (passwordMatch) {
...
exports.forgotPasswordForm = (req, res) => {
  res.render('auth/index', { title: 'Recuperar contraseña', layout: false, activeForm: 'forgot' });
};
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      flash(req, 'success', 'Si el correo está registrado, recibirás un código de recuperación.');
    ...
    try {
      await sendEmail(correo, 'Recuperación de contraseña - BiblioNet', `Tu código de recuperación es: ${codigo}\n\nEste código caduca en 10 minutos.`);
      req.session.resetEmail = correo; 
      flash(req, 'success', `Se ha enviado un código de recuperación al correo: ${correo}`);
      return res.redirect('/recuperar-password/verificar');
    } catch (emailError) {
      flash(req, 'error', 'Error de configuración SMTP. No se pudo enviar el correo de recuperación.');
...
exports.verifyResetCodeForm = (req, res) => {
  ...
  res.render('auth/index', { title: 'Verificar código', layout: false, activeForm: 'verifyResetCode' });
};
...
exports.verifyResetCode = async (req, res, next) => {
  try {
    ...
    if (user.resetAttempts >= 3) {
      flash(req, 'error', 'Demasiados intentos fallidos. Solicita un nuevo código.');
      return res.redirect('/recuperar-password');
    }
    ...
    if (!isMatch) {
      ...
      await user.save();
      flash(req, 'error', `Código incorrecto. Intentos restantes: ${3 - user.resetAttempts}`);
      return res.redirect('/recuperar-password/verificar');
    }
...
exports.resetPasswordForm = (req, res) => {
  if (!req.session.resetEmail || !req.session.resetCodeValidated) return res.redirect('/recuperar-password');
  res.render('auth/index', { title: 'Nueva contraseña', layout: false, activeForm: 'resetPassword' });
};
